...
- `GET /map-layers/route?from=lat,lon&to=lat,lon&modes=bike,walk,transit[&roll=1][&bcycle=1][&plan=<key>][&ebike=1][&stress=quiet|balanced|direct]` — **multi-modal** directions over an in-process A* graph (SRT + bike lanes + PCC stress); `route-stats.json` for graph health. The legacy single `?mode=bike|walk|transit` still works.
  - `ebike=1` rides at 15 mph instead of 9.4 and pays a quarter of the hill cost. `stress=` re-weights the bike penalties (it never removes an edge, so a route always exists) and also sets what earns a "no bike lane" warning. Both default to today's behaviour — `stress=balanced` reproduces the historical weights exactly.
  - Responses carry `climb_ft` (whole trip and per step), `ebike`, and `stress`. `mode` still reports `bike` for an e-bike.
